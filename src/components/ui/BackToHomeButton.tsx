...
const BackToHomeButton: React.FC<BackToHomeButtonProps> = ({ variant = 'fixed' }) => {
  ...
  return (
    <Link 
      to="/"
      className="fixed top-4 left-4 z-50 flex items-center gap-2 px-4 py-2 bg-slate-900/90 hover:bg-slate-800 backdrop-blur-sm border border-slate-700/50 rounded-full text-gray-300 hover:text-white transition-all duration-300 hover:scale-105 shadow-lg"
    >
      <Home className="w-4 h-4" />
